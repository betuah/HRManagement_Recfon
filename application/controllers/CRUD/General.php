        ...
        public function mail($req, $id_cuti, $type) {
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
                    ...
                    $res_mail   = $this->m_crud->get_where('v_general', array('id_general' => $id_cuti));
                    if ($res_mail->num_rows() > 0) {
                        foreach ($res_mail->result() as $get) {  
                            ...
                            $data['type_leave']     = $this->session->bahasa === 'EN' ? $get->name_duty_type_en : $get->name_duty_type_id;
        ...
        public function aproval($act, $id) {
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
                    ...
                    $res_cuti = $this->m_crud->get_where('v_general', array('id_general' => $id));
                    ...
                    if($ex === TRUE) {
                        $this->m_crud->set_data('tb_dtl_umum', 'id_general', $id, $data);
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
                    if ($this->input->post('approver') === null || $this->input->post('approver') === '') {
                        ...
                        $config['upload_path']          = './assets/files/general';
                        $config['allowed_types']        = 'pdf';
                        $config['max_size']             = 5000;
                        $config['file_name']        	= 'file_dinas_'.$uuid;
                    
                        $this->upload->initialize($config);
                        $files_name = null;
                        $error      = array('code' => true);

                        if (!$_FILES['files_general']["error"] == 4) {
                            if ( ! $this->upload->do_upload('files_general')) {
                                $error = array('code' => false , 'error' => $this->upload->display_errors());
                            } else {
                                $files_name = '/assets/files/general/'.$this->upload->data('file_name');
                        ...
                        $data1 = array (
                            ...
                            'id_duty_type'      => $this->input->post('tod'),
                            'others_duty_type'  => $this->input->post('others'),
                            'instansi'          => $this->input->post('inti_name'),
                        ...
                        $data2 = array (
                            ...
                            'notif_en'      => 'New General Duty Submission from '.$this->input->post('general_name_peg').'.',
                            'notif_id'      => 'Pengajuan izin dinas umum baru dari '.$this->input->post('general_name_peg').'.',
                            'stat_notif'    => 0,
                            'pages'         => 'approval',
                            'date'          => date("m/d/Y")
                        );
                        ...
                        $data3 = array (
                            ...
                            'notif_en'      => $this->input->post('general_name_peg').' has submit your name as a delegation.',
                            'notif_id'      => $this->input->post('general_name_peg').' mengajukan nama Anda sebagai delegasinya.',
                            'stat_notif'    => 0,
                            'date'          => date("m/d/Y")
                        );
                        ...
                        if ($error['code'] === false) {
                            ...
                            // $ex = $this->m_crud->create('tb_dtl_umum', $data1);
                            $ex = $this->m_crud->create_trans_notif('tb_dtl_umum', 'tb_notif', $data1, $data2, $data3);
                            if ($ex === TRUE) {
                                $error      = array('code' => true, 'id_general' => $data1['id_general']);
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
                    if ($this->input->post('approver') === null || $this->input->post('approver') === '') {
                        ...
                        $date_ra    = date_create($date_range[0]);
                        $date_rb    = date_create($date_range[1]);
                        $id         = $this->input->post('id_general_edit');

                        $config['upload_path']          = './assets/files/general';
                        $config['allowed_types']        = 'pdf';
                        $config['max_size']             = 5000;
                        $config['file_name']        	= 'file_general_'.$id;
                        ...
                        if (!$_FILES['files_general_edit']["error"] == 4) {
                            if ( ! $this->upload->do_upload('files_general_edit')) {
                                $error = array('code' => false , 'error' => $this->upload->display_errors());
                            } else {
                                $res_general = $this->m_crud->get_where('v_general', array('id_general' => $id));
                                if ($res_general->num_rows() > 0) {
                                    foreach ($res_general->result() as $tmps) {	
                        ...
                        $data = array (
                            ...
                            'start_date'        => date_format($date_ra , "Y-m-d"),
                            'end_date'          => date_format($date_rb , "Y-m-d"),
                            'id_duty_type'      => $this->input->post('tod_edit'),
                            'others_duty_type'  => $this->input->post('others_edit'),
                            'instansi'          => $this->input->post('inti_name_edit'),
                            'lama_hari'         => $this->input->post('count_day_edit'),
                            'id_spv'            => $akses === 2 || $akses === '2' ? 1 : $this->input->post('approver'),
                            'plt'               => $akses === 2 || $akses === '2' ? 1 : $this->input->post('plt_edit'),
                            'ket'               => $this->input->post('statement_edit'),
                            'lokasi'            => $this->input->post('location_edit'),
                            'lampiran'          => $files_name,
                            'year'              => date("Y")
                        );
                        ...
                        $notif1 = array (
                            'id_notif'      => uniqid(),
                            'id_peg'        => $akses === 2 || $akses === '2' ? 1 : $this->input->post('approver'),
                            'notif_en'      => $this->input->post('general_name_peg').' has changed the general duty submission data with ID '.$id.'.',
                            'notif_id'      => $this->input->post('general_name_peg').' telah merubah data pengajuan izin dinas umum dengan ID '.$id.'.',
                            'stat_notif'    => 0,
                            'pages'         => 'approval',
                            'date'          => date("m/d/Y")
                        );
                        ...
                        $notif2 = array (
                            ...
                            'notif_en'      => $this->input->post('general_name_peg').' has changed his delegated.',
                            'notif_id'      => $this->input->post('general_name_peg').' telah merubah delegasinya.',
                            'stat_notif'    => 0,
                            'date'          => date("m/d/Y")
                        );
                        ...
                        $notif3 = array (
                            ...
                            'notif_en'      => $this->input->post('general_name_peg').' has submit your name to become his delegation during his duty.',
                            'notif_id'      => $this->input->post('general_name_peg').' telah mengajukan Anda untuk menjadi delegasinya selama dinas.',
                            'stat_notif'    => 0,
                            'date'          => date("m/d/Y")
                        );
                        ...
                        if ($error['code'] === false) {
                            ...
                            if ($old_plt === $new_plt) {
                                ...
                                if($ex['code'] === 0) {
                                    ...
                                    $error      = array('code' => true, 'id_general' => $id);
                                    echo json_encode($error);
                                } else {
			...
			if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
				...
				if ($decodedToken != false) {
                    ...
                    $notif1 = array (
                        'id_notif'      => uniqid(),
                        'id_peg'        => $spv,
                        'notif_en'      => $nama.' has cenceled the General Duty submission data with ID '.$id.'.',
                        'notif_id'      => $nama.' telah membatalkan data pengajuan cuti dinas/tugas dengan ID '.$id.'.',
                        'stat_notif'    => 0,
                        'pages'         => 'approval',
                        'date'          => date("m/d/Y")
                    );
                    ...
                    $del = $this->m_crud->delete('tb_dtl_umum', array('id_general' => $id));
