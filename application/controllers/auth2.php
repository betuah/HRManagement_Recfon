<?php
	...
	class Auth extends CI_Controller {
		...
		*/
		...
		// Static login (login without SSO)
			...
			if ($res_mail->num_rows() > 0) {
				...
				if ($res->num_rows() > 0) {
					...
					foreach ($res_mail->result() as $data) {	
						...
						$pic 	= $data->pic == '' || $data->pic == null ? $path : $data->pic;
		...
		public function logout() {
			...
			header('Location: http://demo3.seamolec.org');
		}
			...
			if (isset($_SESSION['g_token']) && $_SESSION['g_token']) {
				...
				if ($res->num_rows() > 0) {
					...
					if($domain === 'seameo-recfon.org') {
						$GData = array(
							'id'  			=> $tmp_GData->id,
							'email'     	=> $tmp_GData->email,
							'full_name'		=> $tmp_GData->name,
							'pic'			=> $tmp_GData->picture,
						);
